...
\begin{figure}[t]
    ...
    \caption{Block-wise masking on a Tiny ImageNet image. Green patches indicate visible context provided to the student encoder, while red patches denote masked targets whose embeddings must be predicted.}
    \label{fig:mask_overlay}
\end{figure}

\begin{abstract}
Self-supervised Joint-Embedding Predictive Architectures (JEPAs) learn semantic visual representations without relying on pixel-level reconstruction.
...
The current results provide preliminary evidence that probabilistic heads can model diverse latent predictions, while also exposing open questions on calibration, fairness of compute budgets, and downstream utility.

\end{abstract}

\section{Introduction}
Learning general-purpose visual representations that can be reused across tasks is a central objective in self-supervised learning.
Self-supervised methods that operate directly on raw images are particularly attractive in large-scale settings where labeled data is scarce but unlabeled images are abundant.
...
This latent-space formulation has two advantages: (i) it reduces background leakage and pixel-level shortcuts that plague reconstruction-based pretraining, and (ii) it keeps inductive biases about semantics aligned with downstream recognition tasks where robust object-level features are critical.
...
This report summarizes the design and results of four predictors trained on Tiny ImageNet-100 and compares how each captures multimodal uncertainty in latent feature space.
...
\textbf{Positioning against recent JEPA-denoising literature.}
Recent works (e.g., D-JEPA, 2024/2025) integrate per-token denoising objectives with JEPA-style predictors for generative modeling.
Our setup differs in scope: we focus on representation-oriented evaluation under a fixed JEPA backbone and compare probabilistic heads under matched masking/training infrastructure.
At the same time, these works suggest that tighter joint optimization of JEPA and denoising objectives may improve both stability and sample quality; we discuss this direction as future work.

\textbf{Additional contextual baselines.}
The revision also positions our study relative to strong self-supervised baselines (e.g., DINO-style ViT training), masked-diffusion representation learners (e.g., MDM), and more expressive flow families (MAF/IAF, spline couplings), clarifying which conclusions are currently supported by evidence and which remain hypotheses.
...
\end{equation}
where $z_i$ are teacher targets and $\hat{z}_i$ are predictor outputs at the same masked locations.
For consistency with the implemented setup, the training objective also includes variance and covariance regularization terms in the main runs.
EMA momentum ramps from $0.998$ to $1.0$ over the schedule.
On Tiny ImageNet-100 this baseline reaches 0.24 kNN@1 after processing 10.0M samples (Table~\ref{tab:results}, Figure~\ref{fig:baseline_scheme}), providing a deterministic reference point for the probabilistic heads.
...
\end{equation}
where $c_i$ is the context embedding for target $i$ and $p_\theta$ is computed via the RealNVP change of variables.
On Tiny ImageNet-100 the best run reaches 0.18 kNN@1 after processing approximately 9.3M samples ($1.5\!\times\!10^4$ steps), with validation objective around $-72.83$ in raw training units.
Sampling requires multiple forward passes: each new random base Gaussian sample undergoes the full inverse flow transformation to generate a latent prediction, making flow matching computationally intensive for distribution generation (Figure~\ref{fig:flow_scheme}).
The flow head requires substantial computational investment and remains below diffusion in this setup.
...
kNN@1 represents the percentage of correctly classified validation images using this 20-nearest-neighbor classifier, i.e., a simple proxy for representation quality in embedding space.
...
Notably, our model operates on $64\times64$ images (4× smaller than the original $256\times256$ I-JEPA implementation) with approximately 100× fewer parameters, yet achieves baseline performance of 25\% accuracy under this constrained setup.
...
\subsection{Joint Objective Formulation}
\vspace{2mm}
...
\subsection{Targeted Ablation Experiments}
\vspace{2mm}
...
\begin{itemize}
    ...
    \item \textbf{Evaluation scope.} kNN@1 is a useful online indicator of separability, but by itself it does not establish calibration, robustness, or transfer quality.
...
These results indicate that diffusion is currently the strongest probabilistic head in this setup, though it remains computationally heavier at inference time than single-pass predictors.
At the same time, the present evidence is not yet sufficient to claim broad downstream gains from uncertainty modeling.
The next stage of this project is therefore to validate uncertainty quantitatively (NLL/ECE/entropy analyses), remove compute confounders, and test transfer/robustness with linear-probe and fine-tuned evaluations.
